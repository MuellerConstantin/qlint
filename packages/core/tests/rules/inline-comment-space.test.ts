...
describe('inline-comment-space', () => {
  it('flags every inline-comment spacing problem in the violation fixture', () => {
    const diagnostics = lintFixture('violation', inlineCommentSpace);
  ...
  it('does not flag the clean fixture', () => {
    const diagnostics = lintFixture('clean', inlineCommentSpace);

    expect(diagnostics).toEqual([]);
  });

  it('flags a missing space before an inline line comment', () => {
    const diagnostics = lintRule('LET x = 1;// note\n', inlineCommentSpace);
  ...
  it('does not flag standalone-line comments', () => {
    const diagnostics = lintRule('// header\n    // indented standalone\n/* block */\n', inlineCommentSpace);
  ...
  it('autofixes a missing space by inserting one', () => {
    const result = formatRule('LET x = 1;// note\n', inlineCommentSpace);
  ...
  it('autofixes multiple inline comments in one pass', () => {
    const result = formatRule('LET a = 1;// x\nLET b = 2;   /* y */\n', inlineCommentSpace);
  ...
  it('autofixes two block comments on the same line independently', () => {
    const result = formatRule('LET x = 1;/* one */ /* two */\n', inlineCommentSpace);
