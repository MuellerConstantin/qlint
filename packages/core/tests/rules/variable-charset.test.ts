...
describe('variable-charset', () => {
  it('flags identifiers that fall outside the allowed charset', () => {
    const diagnostics = lintFixture('variable-charset', 'violation', variableCharset);
  ...
  it('does not flag identifiers built from ASCII letters, digits and underscores', () => {
    const diagnostics = lintFixture('variable-charset', 'clean', variableCharset);

    expect(diagnostics).toEqual([]);
  });

  it('accepts a leading underscore', () => {
    const diagnostics = lint('SET _privateVar = 1;', [variableCharset] as const);

    expect(diagnostics).toEqual([]);
  });

  it('accepts dot-separated segments', () => {
    const diagnostics = lint('SET vL.MyVar = 1;\nLET vG.Sys.Path = 2;', [variableCharset] as const);

    expect(diagnostics).toEqual([]);
  });

  it('flags a trailing dot', () => {
    const diagnostics = lint('SET vFoo. = 1;', [variableCharset] as const);
  ...
  it('flags consecutive dots', () => {
    const diagnostics = lint('SET vL..Bar = 1;', [variableCharset] as const);
  ...
  it('flags a segment that starts with a digit', () => {
    const diagnostics = lint('SET vFoo.1Bar = 1;', [variableCharset] as const);
  ...
  it('flags non-ASCII letters', () => {
    const diagnostics = lint("SET v_öäü = 'x';", [variableCharset] as const);
  ...
  it('does not flag system variables assigned via SET / LET', () => {
    const diagnostics = lint("SET DateFormat = 'YYYY-MM-DD';", [variableCharset] as const);
